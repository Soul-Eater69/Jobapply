...
using GPT-4o. Replaces the old Human QA layer with AI quality assurance.
"""
...
class CoverLetterAgent:
    """
    Generates tailored cover letters for each job application using GPT-4o.
    ...
    async def generate(self, job: dict, company_research: Optional[dict] = None) -> dict:
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                max_tokens=1000,
                messages=[
                    {"role": "system", "content": "You are an expert cover letter writer. Write in plain text only."},
...
def _format_profile(profile: dict) -> str:
    ...
    lines = [
        ...
        f"Skills: {', '.join(profile.get('skills', [])[:30])}",
    ]

    if experience:
        lines.append("Experience:")
        for exp in experience[:3]:
            highlights = " | ".join((exp.get("highlights") or exp.get("bullets") or [])[:2])
            lines.append(
...
def _fallback(job: dict, profile: dict) -> str:
    name = f"{profile.get('first_name', '')} {profile.get('last_name', '')}".strip()
    role = profile.get("current_role", "a software professional")
    years = profile.get("years_experience", "several")
